...
Future<Response> onRequest(RequestContext context) async {
  return DatabaseService.startConnection(context, createProject(context));
}
...
Future<Response> createProject(RequestContext context) async {
  ...
  final project = await DatabaseService.projectsCollection.findOne(
    where.eq('project_name', projectName),
  );
  if (project != null) {
    return Response.json(
      statusCode: 400,
      body: {'message': 'Project name is already taken'},
    );
  }
  try {
    ...
    return Response.json(
      statusCode: 201,
      body: {'message': 'Project created successfully', 'project': newProject},
    );
  } catch (e) {
    return Response.json(
      statusCode: 500,
